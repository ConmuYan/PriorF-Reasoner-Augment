...
class CanonicalTrainingSample(BaseModel):
    ...
    ``teacher_prob`` is explicit and cross-checked against the Evidence Card so
    distillation cannot silently switch to teacher logits or another target.
    """
    ...
    def _sample_matches_card(self) -> "CanonicalTrainingSample":
        ...
        if card_teacher_prob is None:
            raise ValueError("canonical trainer requires unmasked teacher_summary.teacher_prob")
...
def _encode_messages(tokenizer: Any, messages: Sequence[Any], *, device: torch.device) -> dict[str, torch.Tensor]:
