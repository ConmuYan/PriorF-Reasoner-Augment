...
def test_evaluate_faithfulness_reuses_frozen_head_path_and_reports_expected_metrics():
    ...
    full_probs = (_sigmoid((10 / 20.0) - 0.2), _sigmoid((11 / 20.0) - 0.2))
    suff_probs = (_sigmoid((0 / 20.0) - 0.2), _sigmoid((1 / 20.0) - 0.2))
    comp_probs = (_sigmoid(((-50) / 20.0) - 0.2), _sigmoid(((-49) / 20.0) - 0.2))
    teacher_probs = (_sigmoid(((-30) / 20.0) - 0.2), _sigmoid(((-29) / 20.0) - 0.2))
    ...
    expected_sufficiency = tuple(full - suff for full, suff in zip(full_probs, suff_probs, strict=True))
    expected_comprehensiveness = tuple(full - comp for full, comp in zip(full_probs, comp_probs, strict=True))
    expected_impact = tuple(full - teacher for full, teacher in zip(full_probs, teacher_probs, strict=True))
    ...
    assert report.decision_flip_rate_sufficiency == pytest.approx(1.0)
    assert report.decision_flip_rate_comprehensiveness == pytest.approx(1.0)
    assert report.decision_flip_rate_teacher_prob_ablation == pytest.approx(1.0)
...
def test_schema_preserving_ablation_keeps_message_shape_but_masks_values_only():
    ...
    assert "teacher_prob: 0.8" in full_text
    assert "teacher_prob: 0.8" in sufficiency_text
...
def test_formal_full_inputs_must_start_unablated():
    ...
    with pytest.raises(ValueError, match="unablated Evidence Cards"):
